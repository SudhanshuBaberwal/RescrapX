...
import { XCircle, AlertTriangle, FileUp, RefreshCw, HelpCircle, ArrowLeft } from 'lucide-react';
...
export default function RejectionApprovalGateway() {
    ...
    const rejectionLogs = [
        { field: 'GSTIN Validation', reason: 'Provided GST number mismatch with the corporate registrar data.' },
        { field: 'RVSF Certificate', reason: 'Uploaded certificate copy is blurred or government authorized signature stamp is missing.' }
    ];
    ...
    const handleReuploadRedirect = () => {
        ...
        setTimeout(() => {
            ...
            router.push(`/partner/verify-documents?email=${encodeURIComponent(email)}&mode=correction`);
        }, 1000);
    };
                ...
                <div className="w-full bg-[#0b0f19]/75 border border-white/[0.08] backdrop-blur-2xl p-6 md:p-8 rounded-3xl shadow-[0_32px_70px_-20px_rgba(0,0,0,0.9)] space-y-6 text-center relative overflow-hidden before:absolute before:inset-0 before:-z-10 before:bg-gradient-to-b before:from-white/[0.03] before:to-transparent animate-in fade-in zoom-in duration-300">
                    ...
                    <div className="pt-2 flex flex-col sm:flex-row items-center gap-3">
                        ...
                        <button
                            ...
                            disabled={isSubmitting}
                            className={`w-full sm:w-2/3 font-semibold text-[11px] py-3.5 px-5 rounded-xl transition-all duration-300 shadow-md text-center tracking-wider uppercase flex items-center justify-center gap-2 group active:scale-[0.98] ${
                                isSubmitting 
