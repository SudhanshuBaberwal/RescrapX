...
export default function PartnerDocumentsPage() {
  ...
  const requiredDocuments = [
    {
      ...
      extra: "Government approved certificate",
    },
    { key: "gst_cert", label: "GST Certificate", extra: "Form GST REG-06" },
    {
      key: "pan_card",
      label: "PAN Card",
      extra: "Company/Firm Permanent Account Number",
    },
    {
      key: "company_reg",
      label: "Company Registration Certificate",
      extra: "COI / Partnership Deed / LLP Agreement",
    },
    {
      key: "bank_details",
      label: "Bank Account Details",
      extra: "Cancelled Cheque / Bank Statement",
    },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const missingDocs = requiredDocuments.filter(
      (doc) =>
        documents[doc.key].status !== "success" || !documents[doc.key].file,
    );
    ...
    try {
      ...
      showToast(
        "RVSF Verification Documents Submitted Successfully!",
        "success",
      );
    } catch (err: any) {
      console.error(err);
      ...
      <div className="w-full max-w-5xl mx-auto my-auto bg-white rounded-3xl shadow-sm border border-gray-100 p-6 sm:p-10 md:p-12 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-6">
          ...
          <div className="text-left sm:text-right">
            <span className="text-[#0B5B32] font-black text-xs uppercase tracking-widest">
              Step 3 of 3
            </span>
        ...
        <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-5 flex gap-4 items-start">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-500 leading-relaxed font-medium">
              We align strictly with standard government audit processes. All
              uploaded documents are insulated, encrypted, and stored directly
              in a protected access vault solely for RVSF verification
              clearance.
            </p>
          </div>
        </div>
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm text-gray-900 font-extrabold block">
                        {doc.label}
                      </span>
                    ...
                    <span className="text-xs text-gray-400 block font-semibold leading-normal">
                      {doc.extra}
                    </span>
                  </div>
                  ...
                  {currentDoc.status === "success" && (
                    <div className="mt-4 flex items-center justify-between gap-3 bg-emerald-50 border border-emerald-100 rounded-xl p-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        <FileText
                          size={20}
                          className="text-[#0B5B32] shrink-0"
                        />
                        <div className="min-w-0">
                          <span className="text-xs font-extrabold text-[#0B5B32] block truncate">
