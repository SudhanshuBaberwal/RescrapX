...
export const proxyRoutes = (target, serviceName = "Service") => {
    if (!target) {
        throw new Error(`[Gateway Error]: Proxy target URL for '${serviceName}' is undefined or empty. Check your Environment Variables on Render.`);
    }
    return proxy(target, {
        parseReqBody: false,
        ...
        userResHeaderDecorator(headers, userReq, userRes) {
            ...
            if (allowedCredentials)
                headers["access-control-allow-credentials"] = String(allowedCredentials);
            if (allowedMethods)
                headers["access-control-allow-methods"] = String(allowedMethods);
            if (allowedHeaders)
                headers["access-control-allow-headers"] = String(allowedHeaders);
            return headers;
        },
    });
};
