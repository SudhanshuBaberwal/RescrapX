...
const proxyOptions = {
  ...
  userResHeaderDecorator(headers: any, userReq: any, userRes: any) {
    ...
    if (allowedCredentials) headers["access-control-allow-credentials"] = allowedCredentials;
    if (allowedMethods) headers["access-control-allow-methods"] = allowedMethods;
...
app.use(
  "/api/auth",
  proxy(env.AUTH_SERVICE_URL, proxyOptions),
);

app.use(
  "/api/notification",
  proxy(env.NOTIFICATION_SERVICE_URL, proxyOptions),
);
app.use("/", router);
