...
export const getAdminDashboard = async (req: Request, res: Response) => {
  try {
    ...
    if (auctionRes.status === "rejected") console.error("Auction Service Error:", auctionRes.reason?.message);
    if (vehicleStatsRes.status === "rejected") console.error("Vehicle Stats Error:", vehicleStatsRes.reason?.message);
    if (vehiclePickupsRes.status === "rejected") console.error("Vehicle Pickups Error:", vehiclePickupsRes.reason?.message);
