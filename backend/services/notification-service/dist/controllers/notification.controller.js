...
const requireFullName = (fullName) => {
    if (typeof fullName !== "string" || !fullName.trim()) {
        throw new Error("Full name is required");
    }
    return fullName.trim();
};
const requireOtp = (otp) => {
    if (!isValidOtp(otp)) {
...
class NotificationController {
    sendVerificationEmail = asyncHandler(async (req, res) => {
        ...
        await notificationService.sendVerificationEmail(email, fullName, otp);
     ...
     */
    sendForgotPasswordEmail = asyncHandler(async (req, res) => {
        const email = requireEmail(req.body?.email);
        const fullName = requireFullName(req.body?.fullName);
        const otp = requireOtp(req.body?.otp);
        await notificationService.sendForgotPasswordEmail(email, fullName, otp);
        return ApiResponse.success(res, {
            statusCode: 200,
     ...
     */
    sendPasswordChangedEmail = asyncHandler(async (req, res) => {
        const email = requireEmail(req.body?.email);
        const fullName = requireFullName(req.body?.fullName);
