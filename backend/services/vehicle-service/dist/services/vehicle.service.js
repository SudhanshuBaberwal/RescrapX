...
import Vehicle, { OwnerPaymentDocumentStatus, OwnerPaymentDocumentType, PartnerDocumentStatus, PartnerDocumentSubmissionStatus, PartnerDocumentType, PaymentStatus, ProcessingStage, RegistrationStep, VehicleStatus, } from "../models/vehicle.model.js";
...
class VehicleService {
    ...
    async calculateVehicleEstimatedPrice(vehicleId) {
        if (!vehicleId) {
            throw new ApiError(400, "Vehicle ID is required");
        }
        if (!mongoose.Types.ObjectId.isValid(vehicleId)) {
    ...
    async reviewPaymentProof(vehicleId, adminId, action, rejectionReason) {
        ...
        if (vehicle.paymentStatus !== PaymentStatus.PROOF_UPLOADED) {
            throw new ApiError(400, `Payment cannot be reviewed because current status is ${vehicle.paymentStatus}`);
        }
        if (action === "APPROVE") {
    ...
    async getVerifiedPaymentVehiclesForOwner(ownerId) {
    ...
    async acceptOfferAndSubmitPaymentDocuments(vehicleId, userId, files) {
        // =========================================================
        // 1. Validate IDs
        // =========================================================
        if (!mongoose.Types.ObjectId.isValid(vehicleId)) {
        ...
        // =========================================================
        if (vehicle.ownerOfferAccepted === true &&
            vehicle.ownerPaymentDocuments &&
            vehicle.ownerPaymentDocuments.length > 0) {
            throw new ApiError(400, "Offer and payment documents have already been submitted");
        }
        // =========================================================
        // 7. Upload all documents to Supabase
        // =========================================================
        const [aadhaarUpload, panUpload, bankProofUpload] = await Promise.all([
            supabaseService.uploadOwnerPaymentDocument(aadhaarFile, vehicleId, "AADHAAR"),
            supabaseService.uploadOwnerPaymentDocument(panFile, vehicleId, "PAN"),
            supabaseService.uploadOwnerPaymentDocument(bankProofFile, vehicleId, "BANK_PROOF"),
        ]);
        // =========================================================
