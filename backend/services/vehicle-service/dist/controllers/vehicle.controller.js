...
import { PartnerDocumentStatus, PartnerDocumentType, } from "../models/vehicle.model.js";
...
export const createVehicleDraft = asyncHandler(async (req, res) => {
    ...
    return ApiResponse.success(res, 201, "Vehicle Draft created successfully", vehicle);
});
export const registerBasicVehicleDetails = asyncHandler(async (req, res) => {
    ...
    const vehicle = await vehicleService.basicDetails(req.user.userId, req.body, vehicleId);
...
export const vehicleCondition = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
    const vehicle = await vehicleService.vehicleCondition(req.user.userId, vehicleId, req.body);
    return ApiResponse.success(res, 201, "vehicle condition saved successfully", vehicle);
});
export const majorComponents = asyncHandler(async (req, res) => {
    const vehicle = await vehicleService.majorComponents(req.user.userId, req.query.vehicleId, req.body);
    return ApiResponse.success(res, 200, "Major Components Saved Successfully.", vehicle);
});
export const uploadVehicleDocumentController = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
    const files = req.files;
    const vehicle = await vehicleService.uploadDocument(req.user.userId, vehicleId, files);
    return ApiResponse.success(res, 200, "Document uploaded successfully.", vehicle);
});
export const uploadVehiclePhotosController = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
    const files = req.files;
    const vehicle = await vehicleService.uploadPhotos(req.user.userId, vehicleId, files);
...
export const vehiclePickupLocation = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
    const vehicle = await vehicleService.savePickupLocation(req.user.userId, vehicleId, req.body);
    return ApiResponse.success(res, 201, "vehicle pickup location set successfully", vehicle);
});
export const reviewVehicle = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
    const vehicle = vehicleService.reviewVehicleAndConfirm(vehicleId);
    return ApiResponse.success(res, 201, "Vehicle Registered Successfully", vehicle);
});
...
export const viewDocument = asyncHandler(async (req, res) => {
    ...
    return ApiResponse.success(res, 201, "Document URL generated successfully", url);
});
export const underVerification = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
    const vehicle = vehicleService.vehicleUnderVerification(vehicleId);
    return ApiResponse.success(res, 201, "Vehicle is under verification", vehicle);
});
export const updateVehicleStatus = asyncHandler(async (req, res) => {
    const { vehicleId } = req.query;
    const { status, rejectionReason } = req.body;
    const vehicle = await vehicleService.handleStatusByAdmin(vehicleId, status, rejectionReason);
    return ApiResponse.success(res, 200, `Vehicle ${status.toLowerCase()} successfully.`, vehicle);
});
export const FindALlVehicleForUserController = asyncHandler(async (req, res) => {
    const userId = req.user?.userId || req.headers["x-user-id"];
    const vehicles = await vehicleService.findAllVehicleOfUser(userId);
    return ApiResponse.success(res, 201, "Vehicle data fetch successfully", vehicles);
});
export const ApplyForBiddingVehicleController = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
...
export const updateAuctionVehicleStatus = asyncHandler(async (req, res) => {
    ...
    const vehicle = await vehicleService.updateAuctionResult(vehicleId, status, auctionId, partnerId ?? null, winningBid != null ? Number(winningBid) : null);
...
export const scheduleVehiclePickup = async (req, res) => {
    try {
        ...
        const vehicle = await vehicleService.scheduledPickup(vehicleId, scheduledAt, pickup, userId);
        return res.status(200).json({
            success: true,
...
export const getVehicleDashboardStats = asyncHandler(async (req, res) => {
    ...
    return ApiResponse.success(res, 200, "Vehicle dashboard data fetched successfully", data);
});
export const getPickupMap = asyncHandler(async (req, res) => {
    try {
        const data = await vehicleRepository.getActivePickupLocations();
        return ApiResponse.success(res, 200, "Pickup locations fetched successfully", data);
    }
    catch (error) {
        console.log(error);
    }
...
export const getPartnerIncomingVehicles = async (req, res) => {
    try {
        const partnerId = req.user?.userId || req.headers["x-user-id"];
        if (!partnerId) {
            return res.status(401).json({
                success: false,
        ...
        const vehicles = await vehicleService.getIncomingVehiclesForPartner(partnerId);
        return res.status(200).json({
            success: true,
...
export const getPartnerProcessingVehicles = asyncHandler(async (req, res) => {
    ...
    const vehicles = await vehicleService.getProcessingVehiclesByPartner(partnerId);
    return ApiResponse.success(res, 200, "Processing vehicles fetched successfully", vehicles);
});
export const getPartnerProcessingStats = asyncHandler(async (req, res) => {
    ...
    return ApiResponse.success(res, 200, "Processing statistics fetched successfully", stats);
});
...
export const verifyPickupOtp = asyncHandler(async (req, res) => {
    ...
    const confirmedBy = req.user?.userId ?? "USER";
    const vehicle = await vehicleService.verifyPickupOtp(vehicleId, otp, confirmedBy);
...
export const getVehicleStatusById = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
...
export const getAllVehiclesWithStatus = asyncHandler(async (req, res) => {
...
export const markVehicleArrived = asyncHandler(async (req, res) => {
    const { vehicleId } = req.body;
    if (!vehicleId) {
        return res.status(400).json({
            success: false,
...
export const uploadPartnerDocument = asyncHandler(async (req, res) => {
    ...
    const document = await vehicleService.uploadPartnerDocument(vehicleId, partnerId, documentType, file);
    return ApiResponse.success(res, 201, "Document uploaded successfully", document);
});
export const submitPartnerDocuments = asyncHandler(async (req, res) => {
    ...
    const vehicle = await vehicleRepository.submitPartnerDocuments(vehicleId, partnerId);
    return ApiResponse.success(res, 200, "Documents submitted for admin approval", vehicle);
});
export const getPartnerDocumentVehicles = asyncHandler(async (req, res) => {
    ...
    return ApiResponse.success(res, 200, "Partner document vehicles fetched successfully", vehicles);
});
export const getPartnerVehicleDocuments = asyncHandler(async (req, res) => {
    ...
    const vehicle = await vehicleService.getPartnerVehicleDocuments(vehicleId, partnerId);
    return ApiResponse.success(res, 200, "Vehicle documents fetched successfully", vehicle);
});
export const getCustomerBookings = asyncHandler(async (req, res) => {
    ...
    return ApiResponse.success(res, 200, "Customer bookings fetched successfully", {
        bookings,
...
export const getCustomerBookingById = asyncHandler(async (req, res) => {
    ...
    const booking = await vehicleService.getCustomerBookingById(userId, vehicleId);
    return ApiResponse.success(res, 200, "Booking details fetched successfully", booking);
});
export const getPartnerDashboard = asyncHandler(async (req, res) => {
    ...
    return ApiResponse.success(res, 200, "Partner dashboard fetched successfully", dashboard);
});
export const updatePartnerProcessingStage = asyncHandler(async (req, res) => {
    ...
    const vehicle = await vehicleService.updatePartnerProcessingStage(vehicleId, partnerId, processingStage);
    return ApiResponse.success(res, 200, "Processing stage updated successfully", vehicle);
});
export const getPartnerDocumentVehiclesForAdmin = asyncHandler(async (req, res) => {
    const vehicles = await vehicleService.getVehiclesWithPartnerDocumentsForAdmin();
    return res.status(200).json({
        success: true,
...
export const reviewPartnerDocument = asyncHandler(async (req, res) => {
    ...
    const vehicle = await vehicleService.reviewPartnerDocument(vehicleId, documentId, status, adminId, rejectionReason);
    return res.status(200).json({
        success: true,
        message: status === PartnerDocumentStatus.APPROVED
...
export const approveAllPartnerDocuments = asyncHandler(async (req, res) => {
    ...
    const vehicle = await vehicleService.approveAllPartnerDocuments(vehicleId, adminId);
    return res.status(200).json({
        success: true,
...
export const reviewPaymentProof = asyncHandler(async (req, res) => {
    ...
    const vehicle = await vehicleService.reviewPaymentProof(vehicleId, adminId, action, rejectionReason);
    return res.status(200).json({
        success: true,
...
export const getOwnerVerifiedPaymentVehicles = asyncHandler(async (req, res) => {
    const ownerId = req.headers["x-user-id"];
    if (!ownerId) {
        throw new ApiError(401, "Unauthorized");
    }
    const vehicles = await vehicleService.getVerifiedPaymentVehiclesForOwner(ownerId);
    return res.status(200).json({
        success: true,
...
export const acceptOfferAndSubmitPaymentDocuments = asyncHandler(async (req, res) => {
    const vehicleId = req.query.vehicleId;
    const userId = req.headers["x-user-id"];
    if (!userId) {
        throw new ApiError(401, "Unauthorized");
