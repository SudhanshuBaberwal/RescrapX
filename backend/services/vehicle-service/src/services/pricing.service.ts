...
import {
  CAR_KERB_WEIGHT_KG,
  type KerbWeightValue,
} from "../data/kerb-weight.dictionary.js";
...
interface WeightLookupResult {
  weightKg: number;
  sourceKey: string;
  match: "EXACT" | "RANGE_MIDPOINT" | "BEST_MATCH";
}
...
class PricingService {
  ...
  private toWeight(value: KerbWeightValue): {
    weightKg: number;
    match: "EXACT" | "RANGE_MIDPOINT";
  } {
    // Single weight
   ...
   */
  private findKerbWeight(vehicle: IVehicle): WeightLookupResult {
    const details = vehicle.vehicleDetails as any;
    ...
    if (!manufacturer && !model && !carName) {
      throw new Error(
        "Vehicle manufacturer/model is required to find kerb weight",
      );
    }
    ...
    if (modelTokens.length === 0) {
      throw new Error(
        `Cannot find kerb weight: vehicle model is missing for manufacturer ${manufacturer || "unknown"}`,
      );
    }
    ...
    if (!best) {
      throw new Error(
        `Kerb weight not found for ${manufacturer || ""} ${model || carName || ""}`.trim(),
      );
    }
  ...
  calculateValuation(vehicle: IVehicle): ValuationResult {
    ...
    adjustments.push(
      `Kerb Weight [${weight.sourceKey}] → ${kerbWeightKg} kg (${weight.match})`,
    );
