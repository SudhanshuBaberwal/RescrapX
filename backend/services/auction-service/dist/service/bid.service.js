...
class BidClass {
    async getMyBids(partnerId) {
        ...
        const auctionMap = new Map(auctions.map((auction) => [auction.auctionId, auction]));
        return bids.map((bid) => {
            const auction = auctionMap.get(bid.auctionId);
            const vehicle = auction?.vehicles?.find((vehicle) => vehicle.vehicleId === bid.vehicleId);
            if (!auction || !vehicle) {
                return {
                    ...bid,
                    status: "UNKNOWN",
            ...
            const isWinner = auction.status === AuctionStatus.ENDED &&
