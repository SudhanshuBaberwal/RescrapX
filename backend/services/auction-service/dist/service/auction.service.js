...
import { AuctionStatus, VehicleAssignedStatus, } from "../models/auction.model.js";
...
async function updateVehicleAuctionResult(vehicleId, status, auctionId, partnerId, winningBid) {
    try {
...
class AuctionService {
    async createAuction(dto, adminId) {
        ...
        try {
            ...
            throw new ApiError(error.response?.status ?? 500, `Vehicle Service Error: ${error.response?.data?.message ?? "Unknown error"}`);
        }
        ...
        try {
            ...
            throw new ApiError(error.response?.status ?? 500, `Partner Service Error: ${error.response?.data?.message ?? "Unknown error"}`);
        }
        ...
        if (auctionPartners.length === 0) {
            throw new ApiError(404, "No partners found within 150 KM of available vehicles.");
        }
    ...
    async getAuctionDataForPartner(partnerId) {
        const auction = await auctionRepository.findActiveAuctionForPartner(partnerId);
        if (!auction) {
            throw new ApiError(404, "No active auction found for this partner.");
        }
        return auction;
    }
    async configureAuctionVehicle(auctionId, dto, adminId) {
        const auction = await auctionRepository.findByAuctionId(auctionId);
        if (!auction) {
            throw new ApiError(404, "Auction not found.");
        }
        if (auction.status !== AuctionStatus.DRAFT) {
            throw new ApiError(400, "Vehicle configuration is allowed only in DRAFT status.");
        }
        const vehicle = auction.vehicles.find((item) => item.vehicleId === dto.vehicleId);
        if (!vehicle) {
        ...
        if (dto.reservePrice < dto.minimumBid) {
            throw new ApiError(400, "Reserve price must be greater than or equal to minimum bid.");
        }
        if (dto.bidIncrement <= 0) {
            throw new ApiError(400, "Bid increment must be greater than 0.");
        }
        const updatedAuction = await auctionRepository.configureVehicle(auctionId, dto.vehicleId, dto.minimumBid, dto.reservePrice, dto.bidIncrement);
        if (!updatedAuction) {
    ...
    async approveAuction(auctionId, adminId) {
        ...
        if (auction.status !== AuctionStatus.DRAFT) {
            throw new ApiError(400, `Auction cannot be approved because its current status is ${auction.status}.`);
        }
        ...
        for (const vehicle of auction.vehicles) {
            if (vehicle.minimumBid == null ||
                ...
                throw new ApiError(400, `Bidding properties are missing for vehicle ${vehicle.vehicleId}.`);
            }
            if (vehicle.minimumBid < 0) {
                throw new ApiError(400, `Minimum bid cannot be negative for vehicle ${vehicle.vehicleId}.`);
            }
            if (vehicle.reservePrice < vehicle.minimumBid) {
                throw new ApiError(400, `Reserve price cannot be less than minimum bid for vehicle ${vehicle.vehicleId}.`);
            }
            if (vehicle.bidIncrement <= 0) {
                throw new ApiError(400, `Bid increment must be greater than 0 for vehicle ${vehicle.vehicleId}.`);
            }
        }
        const now = new Date();
        ...
        const updatedAuction = await auctionRepository.approveAuction(auctionId, adminId);
        if (!updatedAuction) {
    ...
    async checkAuctionsForStartApproval() {
        ...
        for (const auction of auctions) {
            const updated = await auctionRepository.requestAuctionApproval(auction._id.toString());
            if (updated) {
                count++;
            }
        }
    ...
    async approveAuctionStart(auctionId, adminId) {
        ...
        const updatedAuction = await auctionRepository.approveAuctionStart(auctionId, adminId);
        if (!updatedAuction) {
    ...
    async rejectAuctionStart(auctionId, adminId, reason) {
        ...
        if (auction.status !== AuctionStatus.APPROVAL_PENDING) {
            throw new ApiError(400, `Start approval is not pending. Current status: ${auction.status}`);
        }
        const updatedAuction = await auctionRepository.rejectAuctionStart(auctionId, adminId, reason);
        if (!updatedAuction) {
    ...
    async startApprovedAuctions() {
        ...
        let count = 0;
        for (const auction of auctions) {
            const started = await auctionRepository.startApprovedAuction(auction._id.toString());
            if (started) {
                count++;
            }
        }
    ...
    async placeBid(auctionId, vehicleId, partnerId, bidAmount) {
        const auction = await auctionRepository.findByAuctionId(auctionId);
        if (!auction) {
            throw new ApiError(404, "Auction not found.");
        }
        ...
        // Find vehicle
        const vehicle = auction.vehicles.find((item) => item.vehicleId === vehicleId);
        if (!vehicle) {
            throw new ApiError(404, "Vehicle does not belong to this auction.");
        }
        // Check partner
        const partnerExists = auction.partners.some((partner) => partner.partnerId === partnerId);
        if (!partnerExists) {
        ...
        const currentVehiclePrice = vehicle.currentHighestBid && vehicle.currentHighestBid > 0
        ...
        const updatedAuction = await auctionRepository.updateVehicleBid(auctionId, vehicleId, newVehiclePrice, partnerId);
        if (!updatedAuction) {
            throw new ApiError(409, "Bid could not be placed. Vehicle price was updated by another bidder. Please try again.");
        }
        const updatedVehicle = updatedAuction.vehicles.find((item) => item.vehicleId === vehicleId);
        if (!updatedVehicle) {
    ...
    async finalizeAuction(auctionId) {
        const auction = await auctionRepository.closeAuctionAndAssignWinners(auctionId);
        if (!auction) {
        ...
        for (const vehicle of auction.vehicles) {
            ...
            await updateVehicleAuctionResult(vehicle.vehicleId, status, auction.auctionId, vehicle.assignedPartnerId, vehicle.winnerBid);
    ...
    // SINGLE AUCTION
    // ==========================================
    async getAdminAuctionById(auctionId) {
    ...
    // LIVE ACTIVITY
    // ==========================================
    async getAdminAuctionActivity(limit) {
        return auctionRepository.getAdminAuctionActivity(Math.min(Math.max(Number(limit ?? 20), 1), 100));
    }
    // ==========================================
    // CANCEL AUCTION
    // ==========================================
    async cancelAdminAuction(auctionId, adminId, reason) {
        const auction = await auctionRepository.findByAuctionId(auctionId);
        if (!auction) {
            throw new ApiError(404, "Auction not found.");
        }
        ...
        const cancelled = await auctionRepository.cancelAuction(auctionId, adminId, reason ?? "Auction cancelled by admin.");
        if (!cancelled) {
    ...
    async getPartnerWonVehicles(partnerId) {
        ...
        const totalWonValue = vehicles.reduce((total, item) => total +
            (item.vehicle?.winnerBid ?? item.vehicle?.currentHighestBid ?? 0), 0);
        return {
            stats: {
                totalWonVehicles,
                totalWonValue,
    ...
    async getPartnerLiveBiddingOpportunities(partnerId) {
        ...
        for (const auction of auctions) {
            for (const vehicle of auction.vehicles) {
                ...
                opportunities.push({
                    ...
                    timeLeft: Math.max(0, new Date(auction.endTime).getTime() - Date.now()),
                    type: auction.type,
                });
            }
        }
